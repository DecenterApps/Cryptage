...
contract StateManager {
  ...
  function append(bytes _buffer, uint _position, uint _data, uint _nextData, uint _size, uint _nextSize) public pure returns (uint, uint, uint) {
  ...
  function read(bytes _buffer, uint _position, uint _size, uint _nextSize) public pure returns (uint, uint, uint) {
    uint data;
    assembly {
