...
var SubtypePersonFuncPlayMapping = map[string]func(*State, Card, Move){
	...
	"Day Trading Bonus":                     postPlayDayTradingWithMove,
...
var SubtypePersonFuncPreRemoveMapping = map[string]func(*State, Card, Move) error{
	...
	"Day Trading Bonus":                     preRemoveDayTradingWithMove,
...
var SubtypePersonFuncPostRemoveMapping = map[string]func(*State, Card, Move){
	...
	"Day Trading Bonus":                     postRemoveDayTradingWithMove,
...
var ComputerCaseNoSpace = errors.New("computer case no space")
var RigCaseNoSpace = errors.New("rig case no space")
var MountCaseNoSpace = errors.New("mount case no space")
...
var ContainerDoesntExist = errors.New("container does not exist")
...
func playCard(state *State, move Move) error {
	card := config.Cards[move.Card][move.Level]
...
func prePlayCPUMining(state *State, card Card, move Move) error {
	if move.ContainerIndex >= uint(len(state.Locations[move.Location].ComputerCases)) {
		return ContainerDoesntExist
	}
	if card.Level > 0 {
		...
		for i := 0; i < computerCaseMinersCount; i++ {
			for j:= 0; j < computerCaseMinersLevelCount; j++ {
				count += state.Locations[move.Location].ComputerCases[move.ContainerIndex].Count[i][j]
			}
		}

		if count > card.Costs.Space {
			return ComputerCaseNoSpace
		}
	}
...
func prePlayGPUMining(state *State, card Card, move Move) error {
	if move.GpuOption && move.ContainerIndex >= uint(len(state.Locations[move.Location].RigCases)) {
		return ContainerDoesntExist
	}
	if !move.GpuOption && move.ContainerIndex >= uint(len(state.Locations[move.Location].ComputerCases)) {
		return ContainerDoesntExist
	}
	if card.Level > 0 {
		if move.GpuOption {
		...
		if move.GpuOption {
			...
			for i := 0; i < rigCaseMinersCount; i++ {
				for j:= 0; j < rigCaseMinersOffset; j++ {
					count += state.Locations[move.Location].RigCases[move.ContainerIndex].Count[i][j]
				}
			}
...
func prePlayASICMining(state *State, card Card, move Move) error {
	if move.ContainerIndex >= uint(len(state.Locations[move.Location].AsicCases)) {
		return ContainerDoesntExist
	}
	if card.Level > 0 {
		...
		for i := 0; i < asicCaseLevelCount; i++ {
			count += state.Locations[move.Location].AsicCases[move.ContainerIndex].Count[i]
		}
...
func postPlayDeveloperBonus(state *State, card Card, move Move) {
	state.Locations[move.Location].DevelopmentPercentageBonus = uint(math.Floor(float64((100+state.Locations[move.Location].DevelopmentPercentageBonus)*(100+card.Gains.MultiplierDev))/float64(100))) - 100
	diff := uint(math.Floor(float64(state.Locations[move.Location].DevelopmentPercentageBonus*state.Locations[move.Location].Development)/float64(100))) - state.Locations[move.Location].DevelopmentBonus
...
func postPlayDayTradingWithMove(state *State, card Card, move Move) {
	state.DayTradingBonus += card.Gains.MultiplierFunds
}
...
func postPlayProjectTimeDecrease(state *State, card Card) {
	state.ProjectTimePercentageDecrease = uint(math.Ceil(
		float64((100-state.ProjectTimePercentageDecrease)*(100-card.Gains.MultiplierTime)) / float64(100)))
}
...
func postPlayBoostMiningEfficiencyOnLocationWithId(state *State, card Card, location uint) {
	state.Locations[location].MiningPercentageBonus = uint(math.Floor(
		float64((100+state.Locations[location].MiningPercentageBonus)*(100+card.Gains.MultiplierFunds))/float64(100))) - 100
	diff := uint(math.Floor(float64(state.Locations[location].MiningPercentageBonus*state.Locations[location].Mining)/float64(100))) -
...
func preRemoveDayTradingWithMove(state *State, card Card, move Move) error                 { return nil }
...
func preRemoveProject(state *State, card Card, move Move) error { return nil }
...
func postRemoveDayTradingWithMove(state *State, card Card, move Move) {
	state.DayTradingBonus -= card.Gains.MultiplierFunds
}
...
func requirePlayCosts(state *State, card Card, move Move) error {
	...
	if !card.isLocation() && !card.isProject() && !card.isMining() && card.Costs.Space > state.Locations[move.Location].SpaceLeft {
